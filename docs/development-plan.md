...
3. [Phase 1：MVP — 最小可跑通流程](#3-phase-1mvp--最小可跑通流程)
4. [Phase 2：核心打轴体验](#4-phase-2核心打轴体验)
5. [Phase 3：扫色渲染与样式](#5-phase-3扫色渲染与样式)
6. [Phase 4：注音系统](#6-phase-4注音系统)
7. [Phase 5：导出系统](#7-phase-5导出系统)
8. [Phase 6：完善与打磨](#8-phase-6完善与打磨)
9. [Phase 7：跨平台适配与发布](#9-phase-7跨平台适配与发布)
10. [测试策略](#10-测试策略)
11. [风险清单](#11-风险清单)
...
```
Phase 0: 项目初始化        (1-2周)
Phase 1: MVP 可跑通流程     (3-4周)
Phase 2: 核心打轴体验       (3-4周)
Phase 3: 扫色渲染与样式     (3-4周)
Phase 4: 注音系统           (2-3周)
Phase 5: 导出系统           (3-4周)
Phase 6: 完善与打磨         (2-3周)
Phase 7: 跨平台适配与发布   (2-3周)
─────────────────────────────────
总计预估: 19-27周 (约5-7个月，单人全职)
```
...
## 3. Phase 1：MVP — 最小可跑通流程

**时间**：3-4周

**目标**：从导入媒体到手动输入歌词到基本预览，端到端流程可跑通。
...
- [ ] **前端：渲染性能优化**
  - 离屏Canvas缓存（背景条、安全线等不变元素）
  - 文字度量缓存（避免每帧 `measureText()`）
  - 脏矩形检测（只重绘变化区域）
  - FPS监控显示（开发模式）
...
| [FFmpeg drawtext filter](https://ffmpeg.org/ffmpeg-filters.html#drawtext) | 文本叠加滤镜参数 |
| [MDN Canvas API](https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API) | Canvas绘图API参考 |
| [Web Audio API](https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API) | 音频处理（节拍器等） |
